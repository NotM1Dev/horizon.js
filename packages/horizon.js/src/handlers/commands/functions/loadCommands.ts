...
export interface LoadCommandsParams {
  client: Client;
  appCommands?: AppCommandsConfig;
  messageCommands?: {
    directory: string;
  };
}

async function loadAppCommands(config: AppCommandsConfig & { client: Client }) {
...
export async function loadCommands(params: LoadCommandsParams): Promise<void> {
  const { client, appCommands } = params;
